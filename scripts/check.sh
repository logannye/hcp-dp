...
cargo clippy --workspace --all-targets ${VERBOSE:+-v} -- -D warnings

say "Build (${RELEASE:+release} build)"
cargo build --workspace --all-targets ${RELEASE_FLAG:-}

say "Unit tests"
cargo test --workspace --all-targets --quiet ${RELEASE_FLAG:-}
# Guard: fail if no tests detected unless explicitly allowed.
if [[ -z "${ALLOW_ZERO_TESTS:-}" ]]; then
  TOTAL_TESTS=$(cargo test --workspace --all-targets -- --list 2>/dev/null | grep -E '^[^ ]' | wc -l | tr -d ' ')
...
say "Run example: lcs"
LCS_OUT=$(cargo run --quiet --example lcs ${RELEASE_FLAG:-})
...
MC_OUT=$(cargo run --quiet --example matrix_chain ${RELEASE_FLAG:-})
...
NW_OUT=$(cargo run --quiet --example align ${RELEASE_FLAG:-})
...
VIT_OUT=$(cargo run --quiet --example viterbi ${RELEASE_FLAG:-})
...
if [[ -n "${RUN_BENCH}" ]]; then
  ...
  cargo bench ${RELEASE_FLAG:-} || { echo "Bench failed"; exit 1; }
